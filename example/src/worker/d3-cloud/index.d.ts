...
interface Cloud<T extends Word> {
    generate(): { bounds: [{ x: number; y: number }, { x: number; y: number }]; tags: Word[] };
     ...
     */
    canvas(): Cloud<T>;
    canvas(canvasGenerator: () => HTMLCanvasElement | OffscreenCanvas): Cloud<T>;
}

export default function <T extends Word>(): Cloud<T>;
